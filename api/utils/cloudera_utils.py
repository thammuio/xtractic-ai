...
def get_pdf_to_relational_workflow_url() -> Optional[str]:
    """
    Get the URL of the file-to-relational workflow application from Agent Studio
    
    Returns:
        Full URL to the file-to-relational workflow (e.g., https://subdomain.domain.com)
    ...
    try:
        ...
        # Find application with "file-to-relational" in the name
        for app in applications:
            app_name = app.get("name", "").lower()
            if "file-to-relational" in app_name or "pdf to relational" in app_name:
                subdomain = app.get("subdomain")
                if subdomain and domain:
                    return f"https://{subdomain}.{domain}"
        ...
        raise Exception(f"Error getting file-to-relational workflow URL: {str(e)}")
