...
class KnoxAuthFactory:
	...
	def build_session(self) -> requests.Session:
		...
		# Priority: Explicit Cookie -> Knox token (as header for CDP) -> Passcode token -> Basic creds token exchange
		if self.cookie:
			session.headers["Cookie"] = self.cookie
			return session
		
		if self.token:
			# For CDP SSB, Knox JWT tokens must be sent as Authorization Bearer headers
			# NOT as cookies (tested with CDP Public Cloud irb-ssb-test)
